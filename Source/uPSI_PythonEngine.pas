...
a monster template for P4D Tester
  TError as the same on TError = procedure (const Msg: string) of object; in namedpipesimpl
    ...
    add PythonPath: UnicodeString V5.0.2.40   init to V5.0.2.60   add TPathInitializationEvent = procedure ( Sender : TObject; var Path : string ) of Object;
...
begin
  ...
  begin
    ...
    RegisterMethod('Procedure SetPythonPath( const Value: UnicodeString)');
    RegisterMethod('Procedure SetProgramName( const ProgramName : UnicodeString)');
     RegisterMethod('function GetPythonPath: UnicodeString');
    ...
    RegisterMethod('procedure SetProgramArgs2;');
     RegisterMethod('procedure CheckRegistry;');  //get from mX4 version to on OnPathInitialization
    ...
    RegisterProperty('PythonHome', 'UnicodeString', iptrw);
     RegisterProperty('PythonPath', 'UnicodeString', iptrw);
...
//  CL.AddTypeS('binaryfunc', 'integer// will not work');
 ...
 //CL.AddConstantN('DEFAULT_DATETIME_CONVERSION_MODE','').SetString( dcmToTuple);
  ...
  CL.AddTypeS('TSysPathInitEvent', 'Procedure ( Sender : TObject; Path : string)');
...
begin Self.PythonExecutable := T; end;
...
(*----------------------------------------------------------------------------*)
procedure TPythonEnginePythonPath_W(Self: TPythonEngine; const T: UnicodeString);
begin Self.Pythonpath := T; end;

(*----------------------------------------------------------------------------*)
procedure TPythonEnginePythonPath_R(Self: TPythonEngine; var T: UnicodeString);
...
begin
  with CL.Add(TPythonEngine) do begin
   ...
   RegisterMethod(@TPythonEngine.SetProgramArgs2, 'SetProgramArgs2');
     ...
     RegisterMethod(@TPythonEngine.CheckRegistry, 'CheckRegistry');
    ...
    RegisterPropertyHelper(@TPythonEnginePythonPath_R,@TPythonEnginePythonPath_W,'PythonPath');
