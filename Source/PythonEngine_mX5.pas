...
type
  ...
  public
    ...
    procedure DoOpenDll2(const aDllName : string); //override;
    procedure SetProgramArgs2;
    ...
    procedure CheckRegistry;     //get from mX4 version to on OnPathInitialization
...
begin
  ...
  procedure TPythonEngine.Initialize2;
  ...
  const Script =
    ...
    'del sys, path, i, site';
  begin
     if Pythonexecutable {VenvPythonExe} <> '' then
       ExecString(AnsiString(Format(Script, [Pythonexecutable])));
    _path := PySys_GetObject('path');
    if Assigned(FOnSysPathInit) then
      FOnSysPathInit(Self, _path);
  end;
...
procedure TPythonEngine.DoOpenDll2(const aDllName : string);
var
  i : Integer;
begin
  if UseLastKnownVersion then
...
procedure TPythonEngine.SetProgramArgs2;
var
  i, argc : Integer;
