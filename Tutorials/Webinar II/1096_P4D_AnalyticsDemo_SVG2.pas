...
const LoadPy_VCLClass =
 ...
 ANALYTICSSVG='#from delphi_module import svg_image'+LF
    ...
    +'plt.rcParams["figure.figsize"] = 8, 5'+LF
    ...
    +''+LF
    +'plt.show()'+LF
    +'print(df.isnull().sum())'+LF
    +'df = df.dropna()'+LF
    +'X = df.drop("sex", axis=1)'+LF
    +'y = df["sex"]'+LF 
    
    +'from sklearn.cluster import KMeans'+LF
    +'print("CLUSTERING ON CULMEN LENGTH AND CULMEN DEPTH")'+LF
    +'X = df[["bill_length_mm","bill_depth_mm"]]'+LF
    +'kmeans = KMeans(n_clusters=3)'+LF             
    +'kmeans.fit(X)'+LF
    +'y_kmeans = kmeans.predict(X)'+LF
    +'plt.scatter(X.loc[:,"bill_length_mm"], X.loc[:,"bill_depth_mm"],c=y_kmeans,s=50,cmap="viridis")'+LF
    +'centers = kmeans.cluster_centers_'+LF
    +'plt.scatter(centers[:, 0], centers[:, 1], c="black", s=200, alpha=0.5)'+LF
    +'plt.show()';
